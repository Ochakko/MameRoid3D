	...
	private:
		...
		void onLRButtonDown(const MouseEvent& e, bool lButton){
			...
			if( lButton ){
				...
				//右下の隅
				if( canChangeSize &&
					size.x-WINOFFSIZE<=e.localX && size.y-WINOFFSIZE<=e.localY ){
		...
		void paintTitleBar(){
			if( hdcM.hDC==NULL ){
				...
				if( canChangeSize ){
					...
					hdcM.setPenAndBrush(RGB(255, 255, 255),RGB(255, 255, 255));
					Rectangle(hdcM.hDC, size.x-WINOFFSIZE,size.y-WINOFFSIZE,size.x,size.y);
				}
...
//		static const int SIZE_Y= 14;
	...
	public:
		//////////////////// Constructor/Destructor //////////////////////
		OWP_PlayerButton( HBITMAP srcprop, HBITMAP srcscale ){
			BOX_WIDTH= 11;
			SIZE_Y= 15;
			RListener = NULL;
			propbmp = srcprop;
		...
		void draw(){
			...
			for(int i=0; i<8; i++){
				...
				if( i < 6 ){
					pos1x= pos.x+BOX_POS_X+BOX_WIDTH*i;
					pos1y= pos.y+size.y/2-BOX_WIDTH/2;
					pos2x= pos.x+BOX_POS_X+BOX_WIDTH*(i+1)-1;
					pos2y= pos1y+BOX_WIDTH-1;
				}else if( i == 6 ){
					pos1x= pos.x+BOX_POS_X+BOX_WIDTH*i + 2 * BOX_WIDTH;
					pos1y= pos.y+size.y/2-BOX_WIDTH/2;
					pos2x= pos.x+BOX_POS_X+BOX_WIDTH*(i+1)-1 + 2 * BOX_WIDTH;
					pos2y= pos1y+BOX_WIDTH-1;
				}else if( i == 7 ){
					pos1x= pos.x+BOX_POS_X+BOX_WIDTH*i + 2 * BOX_WIDTH;
					pos1y= pos.y+size.y/2-BOX_WIDTH/2;
					pos2x= pos.x+BOX_POS_X+BOX_WIDTH*(i+2)-1 + 2 * BOX_WIDTH;
					pos2y= pos1y+BOX_WIDTH-1;
				}
				...
				case 6: btnPrm= &propset; break;
				case 7: btnPrm= &scaleset; break;
				}
				...
				case 6:
					{
						...
						if( propbmp ){
							HDC hmdc = CreateCompatibleDC( hdcM->hDC ); 
							SelectObject(hmdc, propbmp); 
							BitBlt(hdcM->hDC, x1, y1, (x2 - x1), (y2 - y1), hmdc, 0, 0, SRCCOPY);
							//StretchBlt(hdcM->hDC, x1, y1, (x2 - x1), (y2 - y1), hmdc, 0, 0, 20, 20, SRCCOPY); 
							DeleteDC(hmdc);
						}else{
							MoveToEx(hdcM->hDC, x1+shiftDot,y1+shiftDot, NULL );
							LineTo(hdcM->hDC, x2+shiftDot,y1+shiftDot );
						}
					}
					break;
				...
				case 7:
					{
						...
						if( scalebmp ){
							...
							MoveToEx(hdcM->hDC, x1+shiftDot,y1+shiftDot, NULL );
							LineTo(hdcM->hDC, x2+shiftDot,y1+shiftDot );
							MoveToEx(hdcM->hDC, x1+shiftDot,y2+shiftDot, NULL );
							LineTo(hdcM->hDC, x2+shiftDot,y2+shiftDot );
						}
					}
					break;
		...
		void onLButtonDown(const MouseEvent& e){
			...
			for(int i=0; i<8; i++){
				...
				if( i < 6 ){
					...
					if( BOX_POS_X+BOX_WIDTH*(i+2)<=e.localX && e.localX<BOX_POS_X+BOX_WIDTH*(i+1+2) ){
					}else{
						continue;
					}
				}else if( i == 7 ){
					if( BOX_POS_X+BOX_WIDTH*(i+2)<=e.localX && e.localX<BOX_POS_X+BOX_WIDTH*(i+2+2) ){
					}else{
						continue;
					}
				}
				...
				case 6: _beginthread(drawpropsetButtonUpThread,0,(void *)this); break;
				case 7: _beginthread(drawscalesetButtonUpThread,0,(void *)this); break;
				}
		...
		void setpropsetButtonListener(std::tr1::function<void()> listener){
			propset.buttonListener= listener;
		}
		void setscalesetButtonListener(std::tr1::function<void()> listener){
			scaleset.buttonListener= listener;
		}

		void setRListener(std::tr1::function<void()> listener){
			RListener= listener;
		}
	...
	private:
		...
		public:
			...
			bool buttonPush;
			std::tr1::function<void()> buttonListener;
		}frontPlay,backPlay,stop,reset,frontStep,backStep,propset,scaleset;
		...
		std::tr1::function<void()> RListener;
		...
		static void drawpropsetButtonUpThread(LPVOID	pParam){
			Sleep(100);

			OWP_PlayerButton *thisClass= (OWP_PlayerButton*)pParam;
			thisClass->propset.buttonPush=false;
			thisClass->callRewrite();
		}
		static void drawscalesetButtonUpThread(LPVOID	pParam){
			Sleep(100);

			OWP_PlayerButton *thisClass= (OWP_PlayerButton*)pParam;
			thisClass->scaleset.buttonPush=false;
			thisClass->callRewrite();
		}
	};
	...
	private:
		...
		static const int LABEL_SIZE_X= 55;
	...
	public:
		...
		OWP_Timeline(const std::basic_string<TCHAR> &_name=_T(""), const double &_maxTime=1.0, const double &_timeSize=8.0 ){
			lineData.push_back(new LineData(_name,this,0));
			maxTime= _maxTime;
			timeSize= _timeSize;
			cursorListener= NULL;
			selectListener= NULL;
		...
		struct KeyInfo{
			...
			D3DXVECTOR3 eul, tra;
			CMQOObject* mbase;
			map<CMQOObject*, float> blendweight;
			void *object;
		};
		...
		void draw(){
			...
			//時間軸目盛り
			{
				...
				//hdcM->setPenAndBrush(RGB(240,240,240),NULL);
//change color
				hdcM->setPenAndBrush(RGB(0,255,0),NULL);
...
//引数追加
					lineData[i]->draw(	hdcM,
										...
										timeSize,showPos_time,highLight, currentTime );
				}
			}
			...
			if( dragSelect && dragSelectTime1!=dragSelectTime2 ){
				...
				{//枠描画
					int updrag = 0;
					if( dragSelectLine1 < dragSelectLine2 ){
						updrag = 1;
					}else{
						updrag = 0;
					}
					...
					//hdcM->setPenAndBrush(RGB(min(baseColor.r+20,255),min(baseColor.g+20,255),min(baseColor.b+20,255)),NULL);
//change color
					hdcM->setPenAndBrush(RGB(0, 200, 0),NULL);
					if( xx0<=x0 ){		//左枠
					...
					if( updrag == 1 ){
						...
						if( y1<=yy1 ){		//下枠
							MoveToEx(hdcM->hDC, pos.x+MARGIN, y1, NULL);
							LineTo(hdcM->hDC,   pos.x+MARGIN + size.x-SCROLL_BAR_WIDTH-MARGIN*2, y1);
						}
					}else{
						if( yy0<=y0 ){		//上枠
							MoveToEx(hdcM->hDC, pos.x+MARGIN, y0, NULL);
							LineTo(hdcM->hDC,   pos.x+MARGIN + size.x-SCROLL_BAR_WIDTH-MARGIN*2, y0);
						}
						if( y1<=yy1 ){		//下枠
...
***/
		...
		bool newLine(const std::basic_string<TCHAR>& _name){
			for(int i=0; i<(int)lineData.size(); i++){
				if(lineData[i]->name==_name){
					return false;
				}
			}
			lineData.push_back(new LineData(_name,this,lineData.size()));
		...
		void selectClear(bool noCallListener= false){
			...
			//リスナーコール
			if(!noCallListener && selectListener!=NULL){
				selectListener();
			}
		...
		void onRButtonDown(const MouseEvent& e){
			...
			if( x0<=e.localX && e.localX<x2
			 && y1<=e.localY && e.localY<y2 ){
				propline = showPos_line+ (e.localY-y1)/(LABEL_SIZE_Y-1);
			}

			//時間軸目盛り
			if( x1-2<=e.localX && e.localX<x2
			 && y0<=e.localY && e.localY<y2 ){
				proptime = showPos_time+ (double)(e.localX-x1)/timeSize;
			}
		}
		///	Method : 右マウスボタンアップイベント受信
		void onRButtonUp(const MouseEvent& e){
			...
			if( (propline >= 0) && (proptime >= 0.0) ){
				//リスナーコール
				if( callPropertyListener!=NULL ){
					callPropertyListener( propline, proptime, proppos );
				}
				propline = -1;
				proptime = -1.0;
			}
		...
		void onMouseMove(const MouseEvent& e){
			...
			//カーソルリスナーコール
			if( callCursorListener && cursorListener!=NULL ){
				cursorListener();
			}
		...
		void setCurrentTime(double _currentTime, bool noCallListener=false){
			...
			//リスナーコール
			if( !noCallListener && (cursorListener!=NULL) ){
				cursorListener();
			}
		...
		void setKeyShiftListener(std::tr1::function<void()> listener){
			keyShiftListener= listener;
		}
		///	Accessor : keyShiftListener
		void setKeyDeleteListener(std::tr1::function<void(const KeyInfo&)> listener){
			keyDeleteListener= listener;
		}
		///	Accessor : callPropertyListener
		void setCallPropertyListener(std::tr1::function<void(int targetline, double targettime, POINT targetpos )> listener){
			callPropertyListener= listener;
		}
	...
	private:
		...
		std::tr1::function<void(int targetline, double targettime, POINT targetpos )> callPropertyListener;					//レイヤーのプロパティを呼ぶ時に呼ばれる
		...
		class LineData{
		public:
			LineData(const std::basic_string<TCHAR>& _name, OWP_Timeline *_parent, unsigned int _lineIndex){
				name= _name;
				parent= _parent;
...
//引数追加
			void draw(	HDCMaster *hdcM,
						...
						double timeSize,
						double startTime,
						bool highLight=false, double curt=0.0){
...
//change color
					hdcM->setPenAndBrush(NULL,RGB(min(baseR+60,255),min(baseG+60,255),min(baseB+60,255)));
					Rectangle(hdcM->hDC,x0,y0,x1,y1);
				}
...
//change color and Rect-->Line
				...
				MoveToEx(hdcM->hDC,x1,y0, NULL );
				LineTo(hdcM->hDC, x1, y1 );
				if( highLight ){
					hdcM->setPenAndBrush(RGB(0, 255, 0),RGB(baseR,baseG,baseB));
				}else{
				...
				for(int i=0; i<(int)key.size(); i++){
					...
					if( x1<=xx1 && key[i]->select ){
						...
						if( x1<=xx1-1 && xx0+1<=x2 ){
							//hdcM->setPenAndBrush(NULL,RGB(min(baseR+20,255),min(baseG+20,255),min(baseB+20,255)));
//change color
							hdcM->setPenAndBrush(NULL,RGB(0, 120, 0));
...
//change color								
								if( highLight && ((key[i]->time >= (curt - TIME_ERROR_WIDTH)) && (key[i]->time <= (curt + TIME_ERROR_WIDTH))) ){
...
//						shiftKey(shiftTime,i);
		...
		static const int SCROLL_BAR_WIDTH= 10;//10
				...
				if( targetIndex<=lineData.size()-1 ){
					...
					if( e.localX<x0+LABEL_SIZE_Y ){
						...
						if( changeVisibleListener!=NULL ){
							changeVisibleListener(targetIndex);
						}
					...
					//ロックボタン
						...
						//リスナーコール
						if( changeLockListener!=NULL ){
							changeLockListener(targetIndex);
						}
						dragLockButton= true;
		...
		void onLButtonUp(const MouseEvent& e){
			...
			if( dragLine ){
				...
				if( beforIndex!=afterIndex ){
					...
					//リスナーコール
					if(lineShiftListener!=NULL){
						lineShiftListener(beforIndex,afterIndex);
					}
		...
		void onRButtonUp(const MouseEvent& e){
			...
			if( mouseRBtnOnIndex != -1
			 ...
			 && y0<=e.localY && e.localY<y1 ){
				...
				//リスナーコール
				if( callPropertyListener!=NULL ){
					callPropertyListener(mouseRBtnOnIndex);
				}
			}
			...
			for(unsigned int i=0; i<lineData.size(); i++){
				if( lineData[i]->name==_name ){
					...
					if( lineData[i]->visible!=value ){
						...
						//リスナーコール
						if( !noCallListener && changeVisibleListener!=NULL ){
							changeVisibleListener((int)i);
						}
			...
			for(unsigned int i=0; i<lineData.size(); i++){
				if( lineData[i]->name==_name ){
					...
					if( lineData[i]->lock!=value ){
						...
						//リスナーコール
						if( !noCallListener && changeLockListener!=NULL ){
							changeLockListener(i);
						}
