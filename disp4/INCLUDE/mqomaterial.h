...
public:
	...
	int CreateTexture( WCHAR* dirname );
	int CreateTexture( CPmCipherDll* cipher, WCHAR* dirname );


	CMQOMaterial operator= (const CMQOMaterial &q);
	CMQOMaterial operator* (float srcw) const;
	CMQOMaterial operator*= (float srcw);
