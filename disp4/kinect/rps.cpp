...
int CRps::InitParams()
{
	...
	ZeroMemory( m_armp, sizeof( CMotionPoint* ) * SKEL_MAX );

	return 0;
}
int CRps::DestroyObjs()
{
...
int CRps::CreateParams()
{
	DestroyObjs();
	InitParams();

	CallF( CreateRpsElem(), return 1 );
...
***********************************************************/
/***
int CRps::AvrgMotion( int hsid, TSELEM* tsptr, int type, int filter_size ){
...
int CRps::OnMabiki( float rotval, float mvval )
{
	int ret;
	if( !m_traq || !m_pelem ){
		return 0;
...
***/
/***
int CRps::ResetOutputFlag( D3DXVECTOR3* keyeul, D3DXVECTOR3* keymv )
{
	int frameno, skelno;
	for( frameno = 0; frameno < m_framenum; frameno++ ){
		for( skelno = 0; skelno < SKEL_MAX; skelno++ ){
...
int CRps::SetSym( int skelno, int sksym, TSELEM* tsptr, CModel* srcmodel )
{
	int ret;
	...
	ret = CalcSymPose( tsptr + skelno, srcmodel, traq2, &newq2, &newtra2, &neweul2, befeul2 );
	_ASSERT( !ret );
	if( skelno != sksym ){
		ret = CalcSymPose( tsptr + sksym, srcmodel, traq1, &newq1, &newtra1, &neweul1, befeul1 );
		_ASSERT( !ret );
	}
