...
int CMQOFile::LoadMQOFileFromPnd( LPDIRECT3DDEVICE9 pdev, CPmCipherDll* cipher, int mqoindex, float multiple, CModel* srcmodel )
{
	m_pdev = pdev;
	m_modelptr = srcmodel;
	...
	CallF( SetBuffer( cipher, mqoindex ), return 1 );
...
//////////
	...
	CallF( PickUpMorph(), return 1 );
	CallF( m_modelptr->SetMorphBaseMap(), return 1 );
...
int CMQOFile::SetBuffer( CPmCipherDll* cipher, int mqoindex )
{
	PNDPROP prop;
	...
	newbuf = (unsigned char*)malloc( sizeof( unsigned char ) * ( prop.sourcesize + 1 ) );
	if( !newbuf ){
		_ASSERT( 0 );
		return 1;
	}
	ZeroMemory( newbuf, sizeof( unsigned char ) * ( prop.sourcesize + 1 ) );

	int getsize = 0;
	CallF( cipher->Decrypt( prop.path, newbuf, prop.sourcesize, &getsize ), return 1 );
...
int CMQOFile::PickUpMorph()
{
	map<int,CMQOObject*> tmpobj;
	...
	for( itrobj = m_modelptr->m_object.begin(); itrobj != m_modelptr->m_object.end(); itrobj++ ){
		...
		if( curobj ){
			int cmp0;
			cmp0 = strncmp( curobj->m_name, "elem:", 5 );
			if( cmp0 == 0 ){
				char* sepptr = strchr( curobj->m_name, '-' );
				if( sepptr ){
					char targetname[256];
					strcpy_s( targetname, 256, sepptr + 1 );

					map<int,CMQOObject*>::iterator itrobj2;
					for( itrobj2 = m_modelptr->m_object.begin(); itrobj2 != m_modelptr->m_object.end(); itrobj2++ ){
						CMQOObject* cmpobj = itrobj2->second;
						if( cmpobj ){
							int cmp2 = strcmp( cmpobj->m_name, targetname );
							if( cmp2 == 0 ){
								cmpobj->m_morphobj[ curobj->m_objectno ] = curobj;
								tmpobj[ curobj->m_objectno ] = curobj;
								break;
							}
						}
					}
	...
	map<int,CMQOObject*>::iterator itrtmp;
	for( itrtmp = tmpobj.begin(); itrtmp != tmpobj.end(); itrtmp++ ){
		int delseri = itrtmp->first;
		map<int,CMQOObject*>::iterator itrdel;
		itrdel = m_modelptr->m_object.find( delseri );
		if( itrdel != m_modelptr->m_object.end() ){
			m_modelptr->m_object.erase( itrdel );
		}
	}
