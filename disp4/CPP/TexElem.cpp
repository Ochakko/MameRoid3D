...
int CTexElem::CreateTexData( LPDIRECT3DDEVICE9 pdev )
{
	...
	if( !m_buf ){
		...
		hr = D3DXCreateTextureFromFileInMemoryEx( pdev, m_buf, m_bufsize, 
			D3DX_DEFAULT, D3DX_DEFAULT, 
			miplevels,
			0, D3DFMT_A8R8G8B8,
			(D3DPOOL)m_pool,
		...
		if( hr == D3D_OK ){
			D3DXIMAGE_INFO info1;
			HRESULT hrsize;
			hrsize = D3DXGetImageInfoFromFileInMemory( m_buf, m_bufsize, &info1 );
			if( hrsize == D3D_OK ){
				m_orgwidth = info1.Width;
				m_orgheight = info1.Height;
			}else{
				DbgOut( L"texelem : CreateTexData : GetImageInfoFromFileInMemory error !!! skip size setting %s\r\n", m_name );
				_ASSERT( 0 );
			}
		}else{
			_ASSERT( 0 );
