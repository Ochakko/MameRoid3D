		...
		if( curmqof->m_pointnum == 3 ){
			if( curn3p ){
				...
				for( indexno = 0; indexno < 3; indexno++ ){
					...
					(curn3p + indexno)->pervert->vno = curmqof->m_index[indexno];
					(curn3p + indexno)->pervert->uvnum = 1;
					(curn3p + indexno)->pervert->uv[0] = curmqof->m_uv[indexno];
					(curn3p + indexno)->pervert->vcolflag = curmqof->m_vcolsetflag;
					(curn3p + indexno)->pervert->vcol = (DWORD)( curmqof->m_col[indexno] & 0xFFFFFFFF );
				}			
			}
			setno++;
	...
	*pleng = 0;
	...
	if( dispv ){
		...
		for( i = 0; i < m_facenum; i++ ){
			m_faceno2optfaceno.push_back( 0 );
		}
	}
	...
	for( n3 = 0; n3 < (m_facenum * 3); n3++ ){
		...
		if( dispv ){
			m_faceno2optfaceno[ curn3p->perface->faceno ] = curfaceno;
		}
		...
		if( befmaterialno != curmaterialno ){
			if( dispv ){
				...
				if( !setmqomat ){
					char mes[1024];
					sprintf_s( mes, 1024, "object: %s にマテリアル設定の無い面があります。", m_nameptr );
					MessageBoxA( NULL, mes, "warning!!!", MB_OK );
...
int CPolyMesh3::CalcInfNoSkin( CBone* applybone )
{
	if( !m_infbone ){
		_ASSERT( 0 );
		return 0;
	...
	INFELEM ie;
	ie.boneno = applybone->m_boneno;
	ie.kind = CALCMODE_NOSKIN0;
	ie.userrate = 1.0f;
	ie.orginf = 1.0f;
	ie.dispinf = 1.0f;
	...
	int pno;
	for( pno = 0; pno < m_orgpointnum; pno++ ){
		CInfBone* curib = m_infbone + pno;
		curib->m_infnum = 1;
		curib->m_infelem[0] = ie;
	}
...
int CPolyMesh3::CalcInf( CMQOObject* thisobj, map<int, CBone*>& srcbone )
{
	if( !m_infbone ){
		_ASSERT( 0 );
		return 0;
	...
	for( itrbone = srcbone.begin(); itrbone != srcbone.end(); itrbone++ ){
		CBone* curbone = itrbone->second;
		if( curbone ){
			int isno;
			for( isno = 0; isno < curbone->m_isnum; isno++ ){
				CInfScope* curis = curbone->m_isarray[isno];
				_ASSERT( curis );

				if( (curis->m_validflag == 1) && (!curis->m_targetobj || (curis->m_targetobj == thisobj)) ){
					...
					for( pno = 0; pno < m_orgpointnum; pno++ ){
						CInfBone* curib = m_infbone + pno;
						int existbone = curib->ExistBone( curbone->m_boneno );
						if( existbone < 0 ){
							D3DXVECTOR3* curp = m_pointbuf + pno;
							incnt = 0;
							justflag = 0;
							CallF( curis->CheckInCnt( curp, &incnt, &justflag ), return 1 );
							if( (justflag != 0) || ((incnt % 2) != 0) ){
								D3DXVECTOR3 vdiff = *curp - curis->m_center;
								float vdist = D3DXVec3Length( &vdiff );
								INFELEM ie;
								ie.boneno = curbone->m_boneno;
								ie.kind = CALCMODE_ONESKIN0;
								ie.userrate = 1.0f;
								ie.orginf = curis->m_tmaxdist - vdist;
								ie.orginf = max( 0.001f, ie.orginf );

								CallF( curib->AddInfElem( ie ), return 1 );	
							}
						}
					}
				}
	...
	for( pno = 0; pno < m_orgpointnum; pno++ ){
		CInfBone* curib = m_infbone + pno;
		CallF( curib->NormalizeInf(), return 1 );
		if( curib->m_infnum == 0 ){
			DbgOut( L"no InfElem : object %d, pno %d pos(%f, %f, %f)!!!!!\r\n", 
				thisobj->m_objectno, pno, (m_pointbuf + pno)->x, (m_pointbuf + pno)->y, (m_pointbuf + pno)->z );
		}
	}
